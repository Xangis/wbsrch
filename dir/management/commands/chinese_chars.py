...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-b', '--blockit', default=False, action='store_true', dest='blockit', help='Block any site that meets the threshold.')
    ...
    def handle(self, *args, **options):
        ...
        max = options['max']
        blockit = options.get('blockit', False)
        print('Blockit is {0}'.format(blockit))
        ...
        results = sorted(domains.items(), key=lambda item: item[1], reverse=False)
        for result in results:
            if result[1] >= threshold:
                print('{0} - {1}'.format(result[1], result[0]))
                printed = printed + 1
                if blockit:
                    print('Blocking {0}'.format(result[0]))
                    bsite = BlockedSite()
                    bsite.url = result[0]
                    bsite.reason = 8
                    bsite.save()
